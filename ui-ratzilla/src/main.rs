use std::{cell::RefCell, collections::{BTreeMap, HashSet}, io::Result, rc::Rc};
...
impl AppState {
    ...
    fn sorted_entity_ids(&self) -> Vec<String> {
        ...
        if !self.search_query.is_empty() {
            ids.retain(|id| id.to_lowercase().contains(&self.search_query.to_lowercase()));
        }
        
        // Apply namespace filter
        if let Some(ref namespace) = self.namespace_filter {
            ids.retain(|id| id.starts_with(&format!("{}/", namespace)));
        }
    ...
    fn delete_entity(&mut self, entity_id: &str) {
        ...
        // Clamp selection
        let count = self.sorted_entity_ids().len();
    ...
    fn cycle_namespace_filter(&mut self) {
        ...
        match &self.namespace_filter {
            None => {
                // Set to first namespace
                self.namespace_filter = namespaces.first().cloned();
            }
            Some(current) => {
                // Find next namespace
                if let Some(pos) = namespaces.iter().position(|ns| ns == current) {
                    if pos + 1 < namespaces.len() {
                        self.namespace_filter = Some(namespaces[pos + 1].clone());
                    } else {
                        // Wrap around to first
                        self.namespace_filter = Some(namespaces[0].clone());
                    }
                } else {
                    // Current namespace not found, reset to first
                    self.namespace_filter = namespaces.first().cloned();
                }
            }
        }
...
fn render_header(f: &mut ratzilla::ratatui::Frame, area: Rect, state: &AppState) {
    let chunks = Layout::default()
        ...
        .constraints([Constraint::Min(20), Constraint::Min(20), Constraint::Length(30)])
        .split(area);
    ...
    // Filter status in middle
    let mut filter_spans = Vec::new();
    if state.search_active {
        filter_spans.push(Span::styled("Search: ", Style::default().fg(Color::Yellow)));
        filter_spans.push(Span::styled(&state.search_query, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)));
        filter_spans.push(Span::styled("█", Style::default().fg(Color::Yellow))); // cursor
    } else if !state.search_query.is_empty() {
        filter_spans.push(Span::styled("Filtered: ", Style::default().fg(Color::Green)));
        filter_spans.push(Span::styled(&state.search_query, Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)));
    }
    
    if let Some(ref namespace) = state.namespace_filter {
        if !filter_spans.is_empty() {
            filter_spans.push(Span::styled(" │ ", Style::default().fg(Color::DarkGray)));
        }
        filter_spans.push(Span::styled("NS: ", Style::default().fg(Color::Cyan)));
        filter_spans.push(Span::styled(namespace, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)));
    }

    let filter_paragraph = if filter_spans.is_empty() {
...
fn render_entity_list(f: &mut ratzilla::ratatui::Frame, area: Rect, state: &mut AppState) {
    ...
    let title = if state.namespace_filter.is_some() || !state.search_query.is_empty() {
        format!(" Entities ({}/{}) ", ids.len(), state.entities.len())
    } else {
...
fn render_help(f: &mut ratzilla::ratatui::Frame, area: Rect, state: &AppState) {
    let help_text = if state.search_active {
        Line::from(vec![
            Span::styled(" Type", Style::default().fg(Color::Yellow)),
            Span::styled(" to search  ", Style::default().fg(Color::DarkGray)),
            Span::styled("Enter", Style::default().fg(Color::Yellow)),
            Span::styled(" lock filter  ", Style::default().fg(Color::DarkGray)),
        ...
        Line::from(vec![
            ...
            Span::styled(" filter NS  ", Style::default().fg(Color::DarkGray)),
            Span::styled("N", Style::default().fg(Color::Yellow)),
            Span::styled(" clear NS  ", Style::default().fg(Color::DarkGray)),
        ])
    };
    ...
    terminal.on_key_event({
        ...
        move |key_event| {
            ...
            if s.search_active {
                ...
                let filtered_entity_count = s.sorted_entity_ids().len();
                match key_event.code {
                    ...
                    KeyCode::Down | KeyCode::Char('j') => {
                        if filtered_entity_count > 0 && s.selected_entity < filtered_entity_count - 1 {
                            s.selected_entity += 1;
                            let selected = s.selected_entity;
                            s.table_state.select(Some(selected));
                        }
                    ...
                    KeyCode::Char('n') => {
                        s.cycle_namespace_filter();
                    }
                    KeyCode::Char('N') => {
                        s.clear_namespace_filter();
                    }
                    _ => {}
                }
            }
